...
class MovieDetailsVC: UIViewController{
    ...
    @objc func watchButtonAction(sender: UIButton!) {
        ...
        if((moviesBool.firstIndex(where: {$0.moviesID == String(movieID)})) != nil){
            context.delete(moviesBool.last!)
            ...
            let yeniMoviesBool = MoviesCategori(context: self.context)
            yeniMoviesBool.moviesID = String(MDM[0].id!)
            verileriKaydet()
            watchButton.backgroundColor = UIColor.yellow
        }
    }
    ...
    func atamalarYapilacak(){
        movieNameLabel.text = MDM[0].name
        dateLabel.text = MDM[0].first_air_date!.convertDatetring_TopreferredFormat(currentFormat: "yyyy-MM-dd", toFormat: "dd MMM yyyy")
        ...
        if (MDM[0].number_of_seasons == nil){
            self.runTimeLabel.text = "N/A"
        }
        else{
            self.runTimeCountLabel.text = String(MDM[0].number_of_seasons!)
        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if moviesBool.firstIndex(where: {$0.moviesID == String(movieID)}) != nil{
            ...
            watchButton.backgroundColor = UIColor.darkGray
    ...
    func verileriKaydet(){
        do{
            ...
            print("context save err",error.localizedDescription)
        }
    }
    
    func verileriGetir(){
        let talep: NSFetchRequest<MoviesCategori> = MoviesCategori.fetchRequest()
        do{
            moviesBool =  try context.fetch(talep)
        }
        catch{
            print("hata")
        }
